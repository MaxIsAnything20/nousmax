...
/* the wordmark the monogram stretches into — each letter stretches in one by one */
...
.sf .intro-morph .ltr{ display:inline-block; transform:scaleX(.12); opacity:0; transform-origin:left center; }
.sf .intro-morph .wm-n{ color:#0d9488; }
.sf .intro-morph .wm-m{ color:#d9a521; }
.sf .intro-morph.play .ltr{ animation:ltrIn .55s cubic-bezier(.3,.8,.25,1) forwards; }
.sf .intro-morph.play .ltr:nth-child(1){ animation-delay:0s; }
.sf .intro-morph.play .ltr:nth-child(2){ animation-delay:.09s; }
.sf .intro-morph.play .ltr:nth-child(3){ animation-delay:.18s; }
.sf .intro-morph.play .ltr:nth-child(4){ animation-delay:.27s; }
.sf .intro-morph.play .ltr:nth-child(5){ animation-delay:.36s; }
.sf .intro-morph.play .ltr:nth-child(6){ animation-delay:.45s; }
.sf .intro-morph.play .ltr:nth-child(7){ animation-delay:.54s; }
@keyframes ltrIn{ from{ transform:scaleX(.12); opacity:0; } to{ transform:scaleX(1); opacity:1; } }
...
.sf:not(.nav-up) .dots, .sf:not(.nav-up) .cue, .sf:not(.nav-up) .wm-brand{ opacity:0; pointer-events:none; }
...
function IntroOverlay({ onReveal, onDone }) {
  ...
  useEffect(() => {
    ...
    const morph = () => {
      ...
      // the monogram splits (N left, M right) and fades as the wordmark takes over
      if (paths[0]) paths[0].animate([{ transform: "translateX(0)" }, { transform: "translateX(-56px)" }], { duration: 850, easing: "cubic-bezier(.4,0,.2,1)", fill: "forwards" });
      if (paths[1]) paths[1].animate([{ transform: "translateX(0)" }, { transform: "translateX(56px)" }], { duration: 850, easing: "cubic-bezier(.4,0,.2,1)", fill: "forwards" });
      markSvg.animate([{ opacity: 1, transform: "scale(1)" }, { opacity: 0, transform: "scale(1.55)" }], { duration: 620, easing: "ease", fill: "forwards" });
      if (morphEl) {
        morphEl.classList.add("play");              // NousMax stretches in letter by letter
        morphEl.animate([{ opacity: 0, transform: "scale(.32)" }, { opacity: 0.16, transform: "scale(1)" }],
          { duration: 1250, easing: "cubic-bezier(.2,.7,.2,1)", fill: "forwards" });
      }
      onReveal && onReveal();                       // hero cascades in blurred, then pulls into focus
    ...
    const reveal = () => {
      ...
      const morphEl = document.querySelector(".intro-morph");
      if (morphEl) morphEl.animate([{ opacity: 0.16 }, { opacity: 0 }], { duration: 680, easing: "ease", fill: "forwards" });
      timers.push(setTimeout(() => onDone && onDone(), 700));   // nav chrome + page watermark fade in
    };
  ...
  return (
    <div className="intro">
      ...
      <div className="intro-morph" aria-hidden="true">
        <span className="ltr wm-n">N</span><span className="ltr wm-n">o</span><span className="ltr wm-n">u</span><span className="ltr wm-n">s</span>
        <span className="ltr wm-m">M</span><span className="ltr wm-m">a</span><span className="ltr wm-m">x</span>
      </div>
...
export default function App() {
  ...
  return (
    <div className={`sf ${theme === "light" ? "light" : ""} ${intro ? "intro-active" : ""} ${booting ? "booting" : ""} ${navUp ? "nav-up" : ""}`}>
      <style>{CSS}</style>

      {intro && <IntroOverlay onReveal={() => setBooting(false)} onDone={() => { setIntro(false); setNavUp(true); }} />}
